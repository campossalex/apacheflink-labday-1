...
## Step 5: Fluss PK Table

Let's create a Fluss table to represent the product reference data.
...
## Step 6: Postgres-cdc to Fluss PK Table

Create a simple job to populate the FLuss table, 
using the `product_lookup` as source and `fluss.app.product_table`as sink.
...
### Step 7: Run a SELECT statement to print master_product:
...
```sql,line-numbers,wrap
SELECT * FROM fluss.app.product_table;
```
...
JOIN fluss.app.product_table
ON purchase_stream.product_id = fluss.app.product_table.product_id
```
